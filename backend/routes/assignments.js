...
router.post("/assignments", async (req, res) => {
  ...
  try {
    ...
    if (courseCheck.rowCount === 0) {
      return res
        .status(400)
        .json({ error: "Invalid course_id: Course does not exist" });
    }
    let queryText, values;

    if (status) {
      queryText =
        "INSERT INTO assignments (course_id, title, due_date, status) VALUES ($1, $2, $3, $4)";
      values = [course_id, title, due_date, status];
    } else {
      queryText =
        "INSERT INTO assignments (course_id, title, due_date) VALUES ($1, $2, $3)";
      values = [course_id, title, due_date];
    }
